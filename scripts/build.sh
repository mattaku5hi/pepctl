...
--deb Build Debian package after successful build (uses scripts/package-deb.sh --fast)
...
$0 --deb # Build in Release mode and generate a .deb in ./dist/
...
main() {
    ...
    if [[ "$PACKAGE_DEB" == "true" ]] && [[ "$BUILD_TYPE" != "Release" ]]; then
        log_warn "--deb requested; forcing Release build for packaging"
        BUILD_TYPE="Release"
    fi

    if [[ "$PACKAGE_DEB" == "true" ]]; then
        local has_pepctl=false
        for t in "${targets[@]}"; do
            if [[ "$t" == "pepctl" ]] || [[ "$t" == "main" ]] || [[ "$t" == "all" ]]; then
                has_pepctl=true
                break
            fi
        done
        if [[ "$has_pepctl" == "false" ]]; then
            targets+=("pepctl")
        fi
    fi
    ...
    if build_targets "${targets[@]}"; then
        ...
        if [[ "$PACKAGE_DEB" == "true" ]]; then
            echo
            log_info "Generating Debian package (.deb)..."
            if ./scripts/package-deb.sh --fast --no-deps; then
                log_success "Debian package generated in: $PROJECT_ROOT/dist/"
            else
                log_error "Debian package generation failed"
                exit 1
            fi
        fi
